			   ...
			   struct rt_info *task)
{
	...
	/************** SH-ST *************/
	//set initial value for temp_deadline equal to deadline
	//temp_deadline can be changed later by different algorithms
	task->temp_deadline=&task->deadline;
        task->temp_period=&task->period;
        task->rt_status=T_NORMAL; //Not suspended, not non-preemptive, not anything else
        task->token=0;  //Just in case RNLP is used
        task->res_cnt=0;        //Default
        task->donor=NULL;       //Default
        task->donee=NULL;       //Default
        INIT_LIST_HEAD(&(task->req_res));
        INIT_LIST_HEAD(&(task->req_mutex_list));        //List of required mutexes
	/************** SH-ST *************/
