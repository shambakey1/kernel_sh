...
/* Convert between long and timespecs */
unsigned long timespec_to_long(const struct timespec *ts);
 ...
 */
int check_task_failure(struct rt_info *task, int flags);
...
void initialize_lists(struct rt_info *task);
void initialize_dep(struct rt_info *task);
...
/********************** SH-ST **********************/
static inline void suspend_task(struct rt_info *r, int reschedule){
    /* set the rt_status of current rt_info to SUSPENDED 
     * IF r not specified, use the current rt_info*/
    if(r){
        r->rt_status=SUSPENDED;
    }
    else{
        (&current->rtinfo)->rt_status=SUSPENDED;
    }
    if(reschedule){
...
static inline void non_preemptive_task(struct rt_info *r, int reschedule){
    /* set the rt_status of current rt_info to NON_PREEMPTIVE 
     * If r is not specified, use the current rt_info */
    if(r){
        r->rt_status=NON_PREEMPTIVE;
    }
    else{
        (&current->rtinfo)->rt_status=NON_PREEMPTIVE;
    }
    if(reschedule){
        //reschedule only if required
        force_sched_event(current);
...
static inline void noraml_task(struct rt_info *r,int reschedule){
    /* set the rt_status of current rt_info to NORMAL 
     * If r is not specified, use the current rt_info */
    if(r){
        r->rt_status=T_NORMAL;
    }
    else{
        (&current->rtinfo)->rt_status=T_NORMAL;
    }
    if(reschedule){
        //reschedule only if required
        force_sched_event(current);
...
static inline void bnoraml_task(struct rt_info* r,int reschedule){
    /* set the rt_status of current rt_info to B_NORMAL 
     * If r is not specified, use the current rt_info */
    if(r){
        r->rt_status=B_NORMAL;
    }
    else{
        (&current->rtinfo)->rt_status=B_NORMAL;
    }
    if(reschedule){
        //reschedule only if required
        force_sched_event(current);
    ...
    /* set the rt_status of current rt_info to SPINNING
     * If r is not specified, use the current rt_info */
    if(r){
        r->rt_status=SPINNING;
    }
    else{
        (&current->rtinfo)->rt_status=SPINNING;
    }
    if(reschedule){
        //reschedule only if required
        force_sched_event(current);
